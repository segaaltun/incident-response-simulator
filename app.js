...
function renderNode(){ const node=state.scenario.nodes[state.current]; nodeTitle.textContent = toneText(tr(node.title), 'title'); nodeText.textContent = toneText(tr(node.text), 'body'); const imageId = state.scenario.id.replace(/-/g, '_'); const imageSrc = node.image || `assets/img/${imageId}_${state.stage}.png`; const illCont = $('node-illustration-container'); const illImg = $('node-image'); if(illCont && illImg) { illImg.onerror = function() { this.onerror = null; this.src = `assets/img/${imageId}_1.png`; }; illImg.src = imageSrc; illCont.classList.remove('hidden'); } alertText.textContent = toneText(`${t().liveAlertPrefix} ${tr(node.alert)}`, 'alert'); settingText.textContent = toneText(tr(node.setting), 'body'); currentText.textContent = toneText(tr(node.current), 'body'); changedText.textContent = changedTextForNode(node); developmentsList.innerHTML = tr(node.developments).map((item)=>`<li>${toneText(item, 'body')}</li>`).join(''); optionsList.innerHTML = tr(node.options).map((item)=>`<li>${toneText(item, 'option')}</li>`).join(''); renderEmailSample(node); collapseAllAccordions(); choicesEl.innerHTML=''; feedbackPanel.classList.add('hidden'); retryBtn.classList.add('hidden'); pendingNext=null; choicesEl.className = `choices choices-count-${Math.min(node.choices.length, 5)}`; const shuffledChoices = [...node.choices].sort(() => Math.random() - 0.5); shuffledChoices.forEach((choice,index)=>{ const btn=document.createElement('button'); btn.className=`choice-btn ${choice.tone}`; btn.innerHTML=`<span class="choice-index">${String(index+1).padStart(2,'0')}</span><span class="choice-copy">${toneText(tr(choice.text), 'option')}</span>`; btn.addEventListener('click',()=>handleChoice(choice)); choicesEl.appendChild(btn); }); renderStats(); startTimer(); }
