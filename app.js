...
const onboardingScenarioTitle = $('onboarding-scenario-title'), onboardingScenarioSummary = $('onboarding-scenario-summary'), nodeTitle = $('node-title'), nodeText = $('node-text'), narrativeBox = $('narrative-box'), narrativeEmbed = $('narrative-embed'), choicesEl = $('choices'), feedbackPanel = $('feedback-panel'), feedbackText = $('feedback-text'), feedbackEmoji = $('feedback-emoji'), feedbackHeading = $('feedback-heading'), rewardStrip = $('reward-strip');
...
function renderEmailSample(node){ const sample = node.emailSample ? tr(node.emailSample) : null; const embedded = !!sample; narrativeBox?.classList.toggle('has-embedded-artifact', embedded); if(!narrativeEmbed) return; if(!sample){ narrativeEmbed.classList.add('hidden'); narrativeEmbed.innerHTML=''; return; } const narrativeLabel = currentLanguage === 'tr' ? 'Senaryonun merkezindeki e-posta' : 'Email at the center of this scenario'; narrativeEmbed.classList.remove('hidden'); narrativeEmbed.innerHTML = `<div class="narrative-embed-head"><span class="content-chip narrative-embed-chip">${escapeHtml(t().exampleMessage)}</span><p class="narrative-embed-copy">${escapeHtml(narrativeLabel)}</p></div>${renderEmailArtifact(sample)}`; }
